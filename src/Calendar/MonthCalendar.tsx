...
function MonthCalendar(props: MonthCalendarProps) {
    ...
    function renderDays(
        ...
        for (let i = 0; i < 6; i++) {
            ...
            for (let j = 0; j < 7; j++) {
                ...
                row[j] = (
                    <div
                        className={c(
                            'calendarMonthBodyCell',
                            {
                                'calendarMonthBodyCellCurrent': item.currentMonth
                            }
                        )}
                        onClick={() => selectHandler?.(item.date)}
                    >
                        {
                            dateRender
                                ...
                                : (
                                    <div
                                        className={'calendarMonthBodyCellDate'}
                                    >
                                        <div
                                            className={c(
                                                'calendarMonthBodyCellDateValue',
                                                {
                                                    'calendarMonthBodyCellDateSelected': value.format('YYYY-MM-DD') === item.date.format('YYYY-MM-DD')
                                                }
                                            )}
                                        >{item.date.date()}</div>
    
                                        <div className={'calendarMonthBodyCellDateContent'}>{dateInnerContent?.(item.date)}</div>
                                    </div>
                                )
                        }
                    </div>
        ...
        return rows.map((row) => {
            return(
                <div
                    className={'calendarMonthBodyRow'}
                >
                    {row}
                </div>
            )
    ...
    return (
        <div className={'calendarMonth'}>
            <div className={'calendarMonthWeekList'}>
                {
                    weekList.map((week) => (
                        <div                                    
                            className={'calendarMonthWeekListItem'}
                            key={week}
                        >
            ...
            <div className={'calendarMonthBody'}>
                {
                    renderDays(
                        allDays,
                        // dateRender,
